...
function ux_tabgroup_vertical( $params, $content = null ) {
	...
	if( is_array( $GLOBALS['tabs'] ) ){
		...
		foreach( $GLOBALS['tabs'] as $key => $tab ){
			...
			$active = $key == 0 ? ' active' : ''; // Set first tab active by default.
			$tabs[] = '<li class="tab'.$current.'"><a href="#panel'.$randomid.$i.'" data-url="'.$tab['gh_url'].'">'.$tab['title'].'</a></li>';
...
function ux_tab( $params, $content = null) {
	...
	$x = $GLOBALS['tab_count'];
	$GLOBALS['tabs'][$x] = array( 'title' => sprintf( $title, $GLOBALS['tab_count'] ), 'content' =>  $content, 'gh_url' => $gh_url );
	$GLOBALS['tab_count']++;
}
