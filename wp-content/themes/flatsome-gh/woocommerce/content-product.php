    ...
    <div class="inner-wrap">
        <a href="<?php the_permalink(); ?>">
            <div class="product-image hover_<?php echo $flatsome_opt['product_hover']; ?>">
                ...
                <?php
                if ($flatsome_opt['add_to_cart_icon'] == "show") :
                    $link = array(
                        'url' => '',
                        'label' => '',
                        'class' => ''
...
if (!isset($flatsome_opt['grid_style']) || $flatsome_opt['grid_style'] == "grid1") {
    ?>
                ...
                <div class="info text-center">
                    ...
                    ?>
                    <p class="name"><?php if($show_specialization == 'Yes' ){ echo $prod_name .'('. $specialization .')-'.$univ_shrt_name;} else { echo $prod_name .' - '.$univ_shrt_name ; } ?> </p>
                    <div class="duration_below_title-list"><span class="short-description_sapn"> <p class="edu-duration-list"><?php echo $duration; ?></p></span> <span class="edu-ins-name"><?php echo "University : " . $university_name->name; ?></span></div>

                    <p><?php if($provider_object->name !=''){ echo "Provider:" . $provider_object->name; }?></p>
                    <p class="edu-duration-grid"><?php// echo $duration; ?></p>
                    <span class="edu_new product_label"><?php if ($label_new == "yes") {
                    echo "New";
                } ?></span>
                    <span class="product_label"><?php if ($scholarship == "yes") {
                    echo "Scholarship";
                } ?></span>
...
// GRID STYLE 3
    ...
    ?> 
                ...
                <?php $product_cats = strip_tags($product->get_categories('|', '', '')); ?>
                                <p class="name"><?php the_title(); ?></p>
                                <h5 class="category"><?php list($firstpart) = explode('|', $product_cats);
            echo $firstpart; ?></h5>
