            ...
            foreach ($rss_items as $item) {
                ...
                if ($offset > 0) {
                    if ($offset == $total) {
						...
						if($publish_date == 1){
							$output .= '<span class="blog_date_gh">'.$item->get_date('d').'<br/>'.$item->get_date('M').'</span>';
						}
...
//                    } else {
                            $output .= "<div class='feed-more'>" . "<a class='button-primary' href='" . esc_url($item->get_permalink()) . "'>Read full story</a>" . "</div>";
//                    }
                        }
                        $output .= "</li>";
                        //------------
...
function admin_page() {
    ...
    <strong><?php _e('more :','rss-this'); ?></strong><?php('Shows or hides a "Read full story" button. Set to 0 to hide button. (default=1 Button showing)','rss-this'); ?><br>
