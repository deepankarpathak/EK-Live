    ...
    class YITH_WooCommerce_Advanced_Reviews
    {
        ...
        public function get_average_rating($product_id)
        {
            ...
            $query = $wpdb->prepare("
                ...
                where pm.meta_key = '{$this->meta_key_rating}' and pm2.meta_key = '{$this->meta_key_product_id}' and pm3.meta_key = '{$this->meta_key_approved}'
                and pm2.meta_value = %d and pm3.meta_value = 1 and po.post_parent = '0'", $product_id);
        ...
        public function enqueue_scripts()
        {
            ...
            wp_register_script("attachments-script", YITH_YWAR_URL . 'assets/js/ywar-attachments' . $suffix . '.js', array(
                'jquery',
                'comment-reply'
            ));
        ...
        {
            $suffix = defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ? '' : '.min';
            wp_enqueue_style('yit-style', YITH_YWAR_ASSETS_URL . '/css/yit-advanced-reviews' . $suffix . '.css');
        }
        ...
        function enqueue_admin_styles_scripts($hook)
        {
            ...
            wp_register_script("ajax-back-end-script", YITH_YWAR_URL . 'assets/js/ywar-back-end' . $suffix . '.js', array(
                'jquery',
                'jquery-blockui'
            ));
