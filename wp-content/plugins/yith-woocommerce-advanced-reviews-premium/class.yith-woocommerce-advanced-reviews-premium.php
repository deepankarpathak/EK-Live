...
class YITH_WooCommerce_Advanced_Reviews_Premium extends YITH_WooCommerce_Advanced_Reviews {
	...
	/**
	 * @var $reviews_to_show how many reviews should be showed
	 */
	protected $reviews_to_show = 0;

	/**
	 * @var $show_load_more add a link to load more reviews, if $reviews_to_show is not 0(show all)
	 */
	protected $show_load_more = 0;

	/**
	 * @var $load_more_type Choose if a "load more" should be shown, with textual link (value 2) or button link (value 3) or don't show it (value 1)
	 */
	protected $load_more_type = 1;

	/**
	 * @var $load_more_text text to be used as a "show more" link caption
	 */
	protected $load_more_text;

	/**
	 * @var $enable_review_rating Let users to rate reviews
	 */
	protected $enable_review_rating = 0;

	/**
	 * @var show_peoples_vote Enable users to see user votes for every review
	 */
	protected $show_peoples_vote = 0;

	/**
	 * @var $enable_review_vote Enable users to vote others reviews
	 */
	protected $enable_review_vote = 0;

	/**
	 * @var $reviews_on_dialog Open reviews filtered by rating in a dialog box
	 */
	protected $reviews_on_dialog = 0;

	/**
	 * @var $enable_reply Enable the "reply to" option
	 */
	protected $enable_reply = false;

	/**
	 * @var $enable_report_inappropriate let users report an inappropriate review
	 */
	protected $enable_report_inappropriate = false;

	/**
	 * @var bool enable the visitators to vote even if not logged
	 */
	protected $enable_visitators_vote = false;
	...
	/**
	 * @var $hide_when_inappropriate Hide automatically a review when someone report it as inappropriate content
	 */
	protected $hide_when_inappropriate = false;

	/**
	 * @var $hide_inappropriate_review_threshold Number of users reporting a review as inappropriate content needed for hiding the review
	 */
	protected $hide_inappropriate_review_threshold = 0;

	/**
	 * @var $show_featured_reviews show featured reviews
	 */
	protected $show_featured_reviews = false;

	/**
	 * @var $featured_reviews_count how much featured reviews to show
	 */
	...
	protected function __construct() {
		...
		/**
		 * Add Premium feature script and style
		 */
		...
		switch ( $action ) {
			case 'add-featured' :
				foreach ( $reviews_list as $review_id ) {
					update_post_meta( $review_id, $this->meta_key_featured, 1 );
				}
			...
			case 'remove-featured' :
				foreach ( $reviews_list as $review_id ) {
					update_post_meta( $review_id, $this->meta_key_featured, 0 );
				}
			...
			case 'add-reply-blocked' :
				foreach ( $reviews_list as $review_id ) {
					update_post_meta( $review_id, $this->meta_key_stop_reply, 1 );
				}
			...
			case 'remove-reply-blocked' :
				foreach ( $reviews_list as $review_id ) {
					update_post_meta( $review_id, $this->meta_key_stop_reply, 0 );
				}
	...
	public function add_bulk_actions( $actions ) {
		...
		$actions['add-featured']         = __( 'Add to featured', 'ywar' );
		$actions['remove-featured']      = __( 'Remove from featured', 'ywar' );
		$actions['add-reply-blocked']    = __( 'Block replies', 'ywar' );
		$actions['remove-reply-blocked'] = __( 'Unblock replies', 'ywar' );
		...
		switch ( $arg ) {
			case 'inappropriate' :
				$params['meta_query'][] = array(
					'key'     => $this->meta_key_inappropriate_count,
					'value'   => 0,
					'compare' => '>',
					'type'    => 'numeric'
				);
	...
	public function add_review_table_views( $views ) {
		$views['inappropriate'] = __( 'Inappropriate content', 'ywar' );
		$views['reply_blocked'] = __( 'Reply blocked', 'ywar' );
		$views['featured']      = __( 'Featured', 'ywar' );
	...
	public function advanced_reviews_report( $reports ) {
		$reports['yith_advanced_reviews'] = array(
			'title'   => __( 'Reviews', 'ywar' ),
			'reports' => array(
				"most_commented" => array(
					'title'       => __( 'By most commented', 'ywar' ),
					'description' => '',
					'hide_title'  => true,
					'callback'    => array( $this, 'get_report' )
				),
				"average_rating" => array(
					'title'       => __( 'By average rate', 'ywar' ),
					'description' => '',
					'hide_title'  => true,
					'callback'    => array( $this, 'get_report' )
				),
				"min_rating"     => array(
					'title'       => __( 'By min rate', 'ywar' ),
					'description' => '',
					'hide_title'  => true,
					'callback'    => array( $this, 'get_report' )
				),
				"max_rating"     => array(
					'title'       => __( 'By max rate', 'ywar' ),
					'description' => '',
					'hide_title'  => true,
					'callback'    => array( $this, 'get_report' )
				)
	...
	public function get_childs_review( $review ) {
		$product_id = get_post_meta( $review->ID, $this->meta_key_product_id, true );
		...
		if ( $current_page < $max_page ) {
			...
			if ( isset( $_POST['stars'] ) ) {
				...
				if ( ( ! isset( $stars ) ) || ( ! is_numeric( $stars ) ) || ( intval( $stars ) < 0 ) || ( intval( $stars ) > 5 ) ) {
					wp_send_json( array( - 1, __( 'Impossible to filter by rate', 'ywar' ) ) );
				}
				$stars = intval( $stars );
				if ( $stars > 0 ) {
	...
	public function add_table_css_class( $classes, $post_id ) {
		$stop_reply = get_post_meta( $post_id, $this->meta_key_stop_reply, true );
	...
	public function advanced_reviews_column_sort( $params, $orderby ) {
		switch ( $orderby ) {
			case $this->custom_column_upvoting :
				$params['meta_key'] = $this->meta_key_upvotes_count;
				$params['orderby']  = 'meta_value_num';
				break;

			case $this->custom_column_downvoting :
				$params['meta_key'] = $this->meta_key_downvotes_count;
				$params['orderby']  = 'meta_value_num';
				break;

			case $this->custom_column_inappropriate :
				$params['meta_key'] = $this->meta_key_inappropriate_count;
				$params['orderby']  = 'meta_value_num';
				break;

			case $this->custom_column_featured :
				$params['meta_key'] = $this->meta_key_featured;
				$params['orderby']  = 'meta_value_num';
				break;

			case $this->custom_column_reply_status :
				$params['meta_key'] = $this->meta_key_stop_reply;
				$params['orderby']  = 'meta_value_num';
				break;
		}
	...
	public function add_custom_column( $columns ) {
		$columns[ $this->custom_column_upvoting ]      = '';//__( 'Upvote count', 'ywar' );
		$columns[ $this->custom_column_downvoting ]    = '';//__( 'Downvote count', 'ywar' );
		$columns[ $this->custom_column_inappropriate ] = __( 'Inappropriate', 'ywar' );
		$columns[ $this->custom_column_featured ]      = __( 'Featured', 'ywar' );
		$columns[ $this->custom_column_reply_status ]  = __( 'Reply blocked', 'ywar' );
	...
	public function fill_advanced_reviews_columns( $column_name, $review_id ) {
		switch ( $column_name ) {
			case $this->custom_column_upvoting:
				$vote = get_post_meta( $review_id, $this->meta_key_upvotes_count, true );

				echo $vote ? $vote : 0;
				break;

			case $this->custom_column_downvoting:
				$vote = get_post_meta( $review_id, $this->meta_key_downvotes_count, true );

				echo $vote ? $vote : 0;

				break;
			case $this->custom_column_inappropriate:
				$segnalation = get_post_meta( $review_id, $this->meta_key_inappropriate_count, true );
				...
				echo sprintf( '<img class="%s" title="%s" src="%s">',
					$css_class,
					__( 'Report for inappropriate content', 'ywar' ),
			...
			case $this->custom_column_featured :
				$featured = get_post_meta( $review_id, $this->meta_key_featured, true );
				...
				echo sprintf( '<a class="%s" href="%s" title="%s" data-featured-status="%d" data-review-id="%d"></a>',
					...
					__( 'Select/Deselect this as a featured review', 'ywar' ),
					$featured ? 1 : 0,
					$review_id );
			...
			case $this->custom_column_reply_status :
				$stop_reply = get_post_meta( $review_id, $this->meta_key_stop_reply, true );
				...
				echo sprintf( '<a class="%s" href="%s" title="%s" data-stop-reply-status="%d" data-review-id="%d"></a>',
					...
					__( 'Select/Deselect to block any reply of this review', 'ywar' ),
					$stop_reply ? 1 : 0,
					$review_id );
	...
	public function set_approve_status( $approved ) {
		return ! $this->review_moderation; //approve it if don't need moderation before being shown
	}
	...
	public function initialize_settings() {
		...
		$this->enable_visitators_vote = ( 'yes' == get_option( 'ywar_enable_visitors_vote' ) ) ? true : false;
	 ...
	 *
	 * @param $review  the product review on which write a reply
	 * */
	public function get_review_actions_section( $review ) {
		...
		if ( $this->enable_report_inappropriate && ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) ) {
			...
			$inappropriate_segnalations = get_post_meta( $review->ID, $this->meta_key_inappropriate_list, true );
			...
			if ( $show_inappropriate_link ) {
				$result_div .= '<a class="review-inappropriate" href="' . esc_url( add_query_arg( array( "flag" => $review->ID ), get_permalink( $product_id ) ) ) . '" title="' . __( 'Flag as inappropriate content', 'ywar' ) . '" data-id_review="' . $review->ID . '"></a>';
			}
			$result_div .= '</div>';
		}
	...
	function get_review_reply_link( $comment_id, $review, $product_id ) {
		...
		$onclick = sprintf( 'return addComment.moveForm( "%1$s", "%2$s", "%3$s", "%4$s" )',
			"comment-{$review->ID}", $comment_id, 'respond', $product_id
		);
		...
		$link = sprintf( "<a class='comment-reply-link' href='%s' onclick='%s' aria-label='%s'>%s</a>",
			...
			$onclick,
			__( 'Reply to a previous review', 'ywar' ),
			__( 'Reply', 'ywar' )
		);
	...
	/**
	 *  Show links to let users write a reply
	 *
	 * @param $review  the product review on which write a reply
	 * */
	public function get_reply_section( $review ) {
		...
		if ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			...
			if ( 0 == $review->post_parent ) {
				...
				$no_reply   = get_post_meta( $review->ID, $this->meta_key_stop_reply, true );
				$product_id = get_post_meta( $review->ID, $this->meta_key_product_id, true );
				$comment_id = get_post_meta( $review->ID, $this->meta_key_comment_id, true );
				...
				if ( 1 != $no_reply ) {
					...
					if ( current_user_can( 'manage_options' ) ) {
						$reply_html .= '<a href="' . esc_url( $this->stop_reply_review_url( $review->ID ) ) . '" ' . $target . ' class="stop-reply button" title="' . __( 'This review cannot receive replies', "ywar" ) . '" data-id_review="' . $review->ID . '">' . __( "Block reply", "ywar" ) . '</a>';
					}
				} else {
					$reply_html = '<span style="color:red">' . __( "This review cannot receive comments.", "ywar" ) . '</span>';
					if ( current_user_can( 'manage_options' ) ) {
						$reply_html .= '<a href="' . esc_url( $this->open_reply_review_url( $review->ID ) ) . '" class="open-reply button" title="' . __( "Allow replies", "ywar" ) . '" data-id_review="' . $review->ID . '">' . __( "Allow reply", "ywar" ) . '</a>';
					}
				}

				$reply_html = '<div class="reply">' . $reply_html . '</div>';
	...
	public function show_expanded_review_content( $review ) {
		...
		if ( $this->enable_attachments ) {
			$review_thumbnails = get_post_meta( $review->ID, $this->meta_key_thumb_ids, true );
		...
		if ( $this->enable_review_vote && ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) ) {
			...
			$vote_meta = get_post_meta( $review->ID, $this->meta_key_votes, true );
			...
			if ( ! is_user_logged_in() && ! $this->enable_visitators_vote ) {
				...
				$voting_yes_url = wp_login_url( $voting_yes_url );
				$voting_no_url  = wp_login_url( $voting_no_url );
			}
...
<a id="vote_no_' . $review->ID . '" class="' . $no_class . '" href="' . $voting_no_url . '" data-vote_review="-1" data-id_review="' . $review->ID . '" title="This reviews is not helpful">&nbsp;</a>';
		...
		$review_content_elements = $review_title . $review->post_content . $thumbnail_div . $div_yes_not . $actions_section;

		return apply_filters( "yith_advanced_reviews_review_content_elements", $review_content_elements, $review_title, $review->post_content, $thumbnail_div, $div_yes_not, $actions_section );
	}
	...
	public function  show_filter_helpful_recent( $product_id ) {
		$message_recent_reviews  = __( "Most recent reviews", "ywar" );
		$message_helpful_reviews = __( "Most helpful reviews", "ywar" );
	...
	public function add_filtering_link( $stars, $product_id, $perc ) {
		if ( isset( $perc ) && ( $perc > 0 ) ) {
			echo '<a title="' . sprintf( _n( 'Reviews with %s star', 'Reviews with %s stars', $stars, 'ywar' ), $stars ) . '" href="' . add_query_arg( array( "stars" => $stars ) ) . '" class="ywar_filter_reviews" data-dialog="' . $this->reviews_on_dialog . '" data-id_product="' . $product_id . '" data-stars="' . $stars . '">';
		}
	}
	...
	public function add_review_header( $review_stats ) {
		...
		if ( $this->enable_review_rating && ( $count = $product->get_rating_count() ) ) {
			$str_stars         = _n( 'star', 'stars', $stars, 'ywar' );
			$reviews_list_text = sprintf( __( '%d reviews with rating of %s %s (%s of %s)', 'ywar' ), $review_stats[ $stars ], $stars, $str_stars, $review_stats[ $stars ], $review_stats['total'] );

			echo '<H3 class="ywar_review_list">' . $reviews_list_text . '</H3><a href="' . remove_query_arg( "stars" ) . '" class="ywar_filter_reviews" data-id_product="' . $product->id . '" data-stars="0">' . __( '(Show unfiltered results)', 'ywar' ) . '</a><hr>';
		}
	}
	...
	public function enqueue_scripts() {
		...
		wp_register_script( "ajax-script", YITH_YWAR_URL . 'assets/js/ywar-script' . $suffix . '.js', array( 'jquery' ) );

		$nonce = wp_create_nonce( $this->get_wp_nonce_text( $post->ID ) );

		$loader = apply_filters( 'yith_advanced_reviews_loader_gif', YITH_YWAR_ASSETS_URL . '/images/loading.gif' );

		wp_localize_script( 'ajax-script', 'ywar', array(
			'nonce_value' => $nonce,
			'loader'      => $loader
		) );
		...
		if ( is_product() ) {
			...
			wp_enqueue_script( 'prettyPhoto-init', $assets_path . 'js/prettyPhoto/jquery.prettyPhoto.init.min.js', array(
				'jquery',
				'prettyPhoto'
			), WC_VERSION, true );
	...
	public function change_featured_status_callback() {
		...
		update_post_meta( $review_id, $this->meta_key_featured, ! $featured_status );
	...
	public function change_reply_status_callback() {
		...
		//  toggle current featured status
		update_post_meta( $review_id, $this->meta_key_stop_reply, ! $stop_reply_status );
	 ...
	 *
	 * @return mixed return an json array of type ("code" => $code, "value" => $value) where code is 1 for succes and value
	 * is a string to show or
	 * code is -1 for user not logged in and value is a url to be redirect.
	 *
	 * @since 1.0
	 * @author Lorenzo Giuffrida
	 */
	public function vote_review_callback() {
		...
		$product_id = get_post_meta( $id_review, $this->meta_key_product_id, true );
		...
		$meta_votes = get_post_meta( $review->ID, $this->meta_key_votes, true );
		...
		update_post_meta( $review->ID, $this->meta_key_votes, $meta_votes );
	...
	public function report_inappropriate_review_callback() {
		...
		// Get yes/not rating from post_meta as array and as total count
		$segnalations    = get_post_meta( $review->ID, $this->meta_key_inappropriate_list, true );
		...
		if ( $add_segnalation ) {
			...
			update_post_meta( $review->ID, $this->meta_key_inappropriate_list, $segnalations );
			$segnalations_count = get_post_meta( $review->ID, $this->meta_key_inappropriate_count, true );
			update_post_meta( $review->ID, $this->meta_key_inappropriate_count, ++ $segnalations_count );
			...
			if ( $this->hide_when_inappropriate ) {
				if ( $segnalations_count >= $this->hide_inappropriate_review_threshold ) {
					//  Set as unapproved
					update_post_meta( $review->ID, $this->meta_key_approved, 0 );
				}
			}
		}
		...
		wp_send_json( array(
			"code"  => 1,
			"value" => __( "Thanks for your message, we will check the content of the review.", "ywar" )
		) );
	}
	 ...
	 * @author Lorenzo Giuffrida
	 */
	private function get_wp_nonce_text(
		$product_id
	) {
...
//************************************************************************
	...
	public function get_review_is_helpful_text( $review_id ) {
		...
		return sprintf( __( '%s of %s people found this review helpful', 'ywar' ), $vote["yes"], $vote["yes"] + $vote["not"] );
	}
	...
	public function get_voting_stats( $review_id ) {
		...
		$vote_comment_meta = get_post_meta( $review_id, $this->meta_key_votes, true );
	...
	public function get_ajax_comments_callback() {
		...
		if ( $show_helpful ) {
			$args['meta_key'] = $this->meta_key_upvotes_count;
		...
		if ( isset( $stars ) && ( $stars > 0 ) ) {
			...
			$reviews_title          = '<h3 class="ywar_review_list">' . sprintf( __( '%d reviews with rate of %d %s (%d of %d)', 'ywar' ),
					$filtered_reviews_count, $stars, $str_stars, $filtered_reviews_count, $unfiltered_reviews_count ) . '</h3>';

			$reviews_title .= "<a href=\"$return_path\" class=\"ywar_filter_reviews\" data-id_product=\"$product_id\" data-stars=\"0\">" . __( '(Show unfiltered results)', 'ywar' ) . "</a><hr>";
		}
	...
	public function reviews_list( $product_id, $args = null, $show_featured = false ) {
		/*
		 * Track review shown as featured and remove it from the review list shown below
		 */
		$featured_list = array();
		if ( $show_featured && $this->show_featured_reviews ) {
			foreach ( $this->get_featured_product_reviews( $product_id, $this->featured_reviews_count ) as $review ) {
				...
				$this->show_review( $review, true, "review-featured" );    //show featured items
			}
		}
	 ...
	 * @param int $add_value 1 for adding a vote to total count of votes of type $rate_value, -1 for removing a vote from total count
	 *
	 * @return void
	 *
	 * @since 1.0
	...
	private function add_remove_rating_to_review( $review_id, $rate_value, $add_value ) {
		...
		if ( 1 == $rate_value ) {
			$count = get_post_meta( $review_id, $this->meta_key_upvotes_count, true );
			if ( ! isset( $count ) ) {
				$count = 0;
			}

			$count += $add_value;
			update_post_meta( $review_id, $this->meta_key_upvotes_count, $count > 0 ? $count : 0 );
		} else if ( - 1 == $rate_value ) {
			$count = get_post_meta( $review_id, $this->meta_key_downvotes_count, true );
			if ( ! isset( $count ) ) {
				$count = 0;
			}

			$count += $add_value;
			update_post_meta( $review_id, $this->meta_key_downvotes_count, $count > 0 ? $count : 0 );
		}
	}
	 ...
	 *
	 * @param $return_path  the link to be used for anchor element
	 * @param string $text the text to be showed for anchor element
	 * @param string $order set current order type (recent or helpful)
	 * @param int $next_page the number of the next page to be showed
	 * @param $product_id   the product id from which the reviews are retrieved
