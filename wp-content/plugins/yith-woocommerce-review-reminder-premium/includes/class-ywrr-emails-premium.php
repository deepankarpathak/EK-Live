        ...
        public function get_review_list( $order_id ) {
            ...
            switch ( $criteria ) {
                ...
                case 'most_reviewed' :
                    ...
                    WHERE     a.order_id = %d AND a.order_item_type = 'line_item'
                    GROUP BY  a.order_item_id
                    ORDER BY (SELECT COUNT(*) FROM wp_comments WHERE comment_post_ID = MAX(CASE WHEN b.meta_key = '_product_id' THEN b.meta_value ELSE NULL END) AND user_id > 0) DESC
                    LIMIT     %d
                    ", $args ) );
                    break;
                case 'least_reviewed' :
                    ...
                    WHERE     a.order_id = %d AND a.order_item_type = 'line_item'
                    GROUP BY  a.order_item_id
                    ORDER BY (SELECT COUNT(*) FROM wp_comments WHERE comment_post_ID = MAX(CASE WHEN b.meta_key = '_product_id' THEN b.meta_value ELSE NULL END) AND user_id > 0) ASC
                    LIMIT     %d
                    ", $args ) );
                    break;
                case 'highest_priced' :
