...
class bresponZive_themepacific_Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'media':
					...
					if(!isset($value['mod'])) $value['mod'] = '';
					$output .= bresponZive_themepacific_Options_Machine::optionsframework_media_uploader_function( $value['id'], $value['std'], $int, $value['mod'] ); // New AJAX Uploader using Media Library			
				break;
		...
		'Zeyada' => 'Zeyada',
				...
				case 'slider':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= bresponZive_themepacific_Options_Machine::optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order,$int);
					} else {
						$i = 0;
	...
	public static function optionsframework_uploader_function($id,$std,$mod){
		...
		$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">'.__('Upload','bresponZive').'</span>';
